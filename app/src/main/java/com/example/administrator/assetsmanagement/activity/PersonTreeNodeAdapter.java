...
import com.example.administrator.assetsmanagement.treeUtil.NodeRecyclerViewAdapter;

import java.util.List;

/**
 * 人员树结构适配器，其特点是不能增加修改删除父节点，只能编辑叶子。父节点不显示选择框。
 * Created by Administrator on 2017/11/11 0011.
 */

public class PersonTreeNodeAdapter extends NodeRecyclerViewAdapter<PersonTreeNodeAdapter.MyViewHolder> {
    TreeNodeSelected selectedListener;

    public <T> PersonTreeNodeAdapter(Context context, List<T> datas, int defaultExpandLevel, int iconExpand, int iconNoExpand) {
        super(context, datas, defaultExpandLevel, iconExpand, iconNoExpand);
    }

    public <T> PersonTreeNodeAdapter(Context context, List<T> datas, int defaultExpandLevel) {
        super(context, datas, defaultExpandLevel);
    }

    @Override
    public void onBindViewHolder(final BaseNode node, RecyclerView.ViewHolder holder, final int position) {
        final MyViewHolder myViewHolder = (MyViewHolder) holder;
        myViewHolder.mNodeName.setText(node.getName());
